...
Elm.Main.make = function (_elm) {
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var SendMessage = F2(function (a,b) {    return {ctor: "SendMessage",_0: a,_1: b};});
   var UpdateMessage = function (a) {    return {ctor: "UpdateMessage",_0: a};};
   var AddMessage = function (a) {    return {ctor: "AddMessage",_0: a};};
   var NoOp = {ctor: "NoOp"};
   var sendMessageMailbox = $Signal.mailbox(NoOp);
   var socketMessage = function (action) {    return A2($Task.map,function (_p0) {    return NoOp;},A2($Signal.send,sendMessageMailbox.address,action));};
   var update = F2(function (action,model) {
      var _p1 = action;
      switch (_p1.ctor)
      {case "NoOp": return {ctor: "_Tuple2",_0: model,_1: $Effects.none};
         case "AddMessage": var message = {userName: model.currentUser,text: model.newMessage};
           return {ctor: "_Tuple2",_0: _U.update(model,{newMessage: "",messages: A2($List._op["::"],message,model.messages)}),_1: $Effects.none};
         case "UpdateMessage": return {ctor: "_Tuple2",_0: _U.update(model,{newMessage: _p1._0}),_1: $Effects.none};
         default: return {ctor: "_Tuple2",_0: model,_1: $Effects.task(socketMessage(action))};}
   });
   var messages = Elm.Native.Port.make(_elm).inboundSignal("messages",
   "Main.Message",
   function (v) {
      return typeof v === "object" && "userName" in v && "text" in v ? {_: {}
                                                                       ...
                                                                       v.text)} : _U.badPort("an object with fields `userName`, `text`",v);
   });
   var incomingActions = A2($Signal.map,AddMessage,messages);
   var stringToChat = function (message) {
      return A2($Html.li,_U.list([]),_U.list([$Html.text(A2($Basics._op["++"],message.userName,A2($Basics._op["++"],": ",message.text)))]));
   };
   var chatMessages = function (messages) {    return A2($Html.ul,_U.list([]),A2($List.map,stringToChat,messages));};
   var onSubmitWithOptions = F3(function (address,newMessage,user) {
      return A4($Html$Events.onWithOptions,
      "submit",
      _U.update($Html$Events.defaultOptions,{preventDefault: true}),
      $Json$Decode.value,
      function (_p2) {
         return A2($Signal.message,address,A2(SendMessage,newMessage,user));
      });
   });
   ...
   var newChatMessage = F3(function (address,newMessage,user) {
      return A2($Html.form,
      _U.list([A3(onSubmitWithOptions,address,newMessage,user)]),
      _U.list([A2($Html.input,_U.list([$Html$Attributes.type$("text"),A2(onInput,address,UpdateMessage),$Html$Attributes.value(newMessage)]),_U.list([]))
              ,A2($Html.input,_U.list([$Html$Attributes.type$("submit"),$Html$Attributes.value("Send")]),_U.list([]))]));
   });
   var view = F2(function (address,model) {
      return A2($Html.div,_U.list([]),_U.list([chatMessages(model.messages),A3(newChatMessage,address,model.newMessage,model.currentUser)]));
   });
   var initialModel = {messages: _U.list([{userName: "me",text: "Hello, World!"}]),currentUser: "mjs2600",newMessage: ""};
   var app = $StartApp.start({init: {ctor: "_Tuple2",_0: initialModel,_1: $Effects.none},view: view,update: update,inputs: _U.list([incomingActions])});
   var main = app.html;
   var defaultMessage = {userName: "",text: ""};
   var processAction = function (action) {
      var _p3 = action;
      if (_p3.ctor === "SendMessage") {
            return {userName: _p3._1,text: _p3._0};
         } else {
            return defaultMessage;
         }
   };
   var addComments = Elm.Native.Port.make(_elm).outboundSignal("addComments",
   function (v) {
      return {userName: v.userName,text: v.text};
   },
   A2($Signal.map,processAction,sendMessageMailbox.signal));
   var Model = F3(function (a,b,c) {    return {messages: a,currentUser: b,newMessage: c};});
   var Message = F2(function (a,b) {    return {userName: a,text: b};});
